// VSOP87_Test.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
double Sun(double d, double &L, double &M)
{
    ...
    double E = M + (180 / M_PI) * e * sin(Radians(M)) * (1 + e * cos(Radians(M)));

    double x = cos(Radians(E)) - e;
    double y = sin(Radians(E)) * sqrt(1 - e * e);
...
// return value lonm
double Moon(double d, double &lonm, double &lons)
{
    ...
    double E0 = M + (180 / M_PI) * e * sin(Radians(M)) * (1 + e * cos((Radians(M))));
    double E1 = E0 - (E0 - (180 / M_PI) * e * sin(Radians(E0)) - M) / (1 - e * cos(Radians(E0)));
    double E = E1;;

    double x = a * (cos(Radians(E)) - e);
    double y = a * sqrt(1 - e * e) * sin(Radians(E));
    ...
    double lon = Degrees(atan2(yeclip, xeclip));
    double lat = Degrees(atan2(zeclip, sqrt(xeclip * xeclip + yeclip * yeclip)));
    ...
    double Plon = -1.274 * sin(Radians(Mm - 2 * D))    // (Evection)
                  + 0.658 * sin(Radians(2 * D))         // (Variation)
                  - 0.186 * sin(Radians(Ms))            // (Yearly equation)
                  ...
                  - 0.035 * sin(Radians(D))            // (Parallactic equation)
...
double ymdhms2jd(int y, int m, int d, int hh, int mm, int ss)
{
    int CalMode = 1;
...
int main()
{
    ...
    //double u = abs(ISOymd);
    //double w = floor(u / 10000);

    //double y = YearSignVal * w;
    //double m = floor((u - 10000 * w) / 100);
    //double d = u - 10000 * w - 100 * m;
    ...
    //JD += -0.5 + (3600 * hh + 60 * mm + ss) / 86400;
    
    time_t tim = time(nullptr);
    struct tm gm;
    gmtime_s(&gm, &tim);
    double JD = ymdhms2jd(gm.tm_year+1900, gm.tm_mon+1, gm.tm_mday, gm.tm_hour, gm.tm_min, gm.tm_sec);
    for (int n = 0; n < 10; n++)
    {
        double d = (JD+n) - 2451543.5;
        double t = (JD+n - 2451545.0) / 365250;
        ...
        std::cout << std::setprecision(10) << "Mercury=" << Me << " Venus=" << Ve << " Earth=" << Ea << " EarthRot=" << Su << " Moon=" << Mo << " Mars=" << Ma << " Jupiter=" << Ju << "\n";
        //std::cout << std::setprecision(6) << "Mars_L=" << Mars_B_L0(t) << " " << Mars_B_L1(t) << " " << Mars_B_L2(t) << " " << Mars_B_L3(t) << " " << Mars_B_L4(t) << " " << Mars_B_L5(t) << "\n";
    }
